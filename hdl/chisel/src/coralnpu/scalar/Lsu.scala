...
class LsuCell(p: Parameters) extends Bundle {
  ...
  def next(
    ...
    val doInvalidate = vectorMask.map{ x =>
      x.valid && !x.bits
    }.getOrElse(false.B)
    val doData = vectorMask.map{ x =>
      x.valid && x.bits
    }.getOrElse(false.B)
    ...
    val noBadWb = !doWb || state.isOneOf(LsuCellState.W_WB, LsuCellState.DONE) || (write && state.isOneOf(LsuCellState.W_START, LsuCellState.W_RESP))
    // Wb must not be skipped when reading
    val noBadSkipWb = write || !respData.valid || !wb
    val noBadTransitions = noBadInvalidate && noBadData && noBadStart && noBadResp && noBadWb && noBadSkipWb

    // Check for unused inputs
    val noUnusedIndex = vectorIndex.map(!_.valid || doData || doInvalidate).getOrElse(true.B)
    val noUnusedData = vectorData.map(!_.valid || (write && (doData || doInvalidate))).getOrElse(true.B)
...
class LsuSuperSlot(p: Parameters) extends Module {
  ...
  class State extends Bundle {
    ...
    def maybeStart(): (ValidIO[BusReq], UInt, UInt) = {
      ...
      def maybeStartNormal(window: Vec[LsuCell]): (Bool, UInt, UInt, UInt, UInt) = {
        ...
        val wData = VecInit.tabulate(p.lsuDataBytes) { j =>
          // Priority mux, slow
          MuxCase(WireInit(UInt(8.W), DontCare), (0 until window.length).map{ i =>
            bundle(i)(j) -> window(i).data
          })
        }.asUInt
        val wMask = bundle.reduce(_ | _)
        val started = VecInit.tabulate(nCells) { i =>
          cellCanStart(i) &&
      ...
      def maybeStartStrict(window: Vec[LsuCell]): (Bool, UInt, UInt, UInt, UInt) = {
        ...
        val wData = VecInit.tabulate(p.lsuDataBytes) { j =>
          ...
          MuxUpTo1H(WireInit(UInt(8.W), DontCare), (0 until window.length).map{ i =>
            bundle(i)(j) -> window(i).data
          })
        }.asUInt
        val wMask = bundle.reduce(_ | _)
      ...
      val (reqValidNormal, wDataNormal, wMaskNormal, startedNormal, moveLeadNormal) = maybeStartNormal(windowFn(windowSizeNormal))
      val (reqValidStrict, wDataStrict, wMaskStrict, startedStrict, moveLeadStrict) = maybeStartStrict(windowFn(windowSizeStrict))
    ...
    def maybeWriteback(): (WritebackReq, UInt) = {
      ...
      val scalarReq = MuxLookup(scalarWritebackMode, MakeInvalid(UInt(32.W)))(Seq(
      ...
      val scalarWritebacks = VecInit.tabulate(nCells) { i =>
        if (i < 4) {
          (scalarWritebackMode =/= LsuScalarWritebackMode.NONE) && (cells(i).state === LsuCellState.W_WB)
        } else {
          false.B
        }
      }.asUInt
      ...
      val req = MakeWireBundle[WritebackReq](
          new WritebackReq,
          _.integer -> Mux(float.map(_.writeback).getOrElse(false.B), MakeInvalid(UInt(32.W)), scalarReq),
      )
      req.float.foreach { x =>
        x := Mux(float.get.writeback, scalarReq, MakeInvalid(UInt(32.W)))
      }
      val vectorWritebackValid = Option.when(p.enableRvv) {
        vector.get.writebackActiveCells.valid &&
        vector.get.writebackActiveCells.bits.map { x =>
          // TODO: subvector mask
      ...
      req.vector.foreach { x =>
        val vectorWbData = VecInit.tabulate(p.rvvVlenb) { i =>
      ...
      val writebacks = MuxUpTo1H(0.U, Seq(
        (scalarWritebackMode =/= LsuScalarWritebackMode.NONE) -> scalarWritebacks,
        vector.map(_.writebackActiveCells.valid).getOrElse(false.B) -> vectorWritebacks.getOrElse(0.U),
      ))
    ...
    def act(
      ...
      val vectorDataActive = Option.when(p.enableRvv) {
        val enableLane = MuxLookup(
            ...
            "b01_11".U -> Cat(0.U((p.rvvVlenb / 2).W), ~0.U((p.rvvVlenb / 4).W), 0.U((p.rvvVlenb / 4).W)),  // 2 of 4
            "b10_11".U -> Cat(0.U((p.rvvVlenb / 4).W), ~0.U((p.rvvVlenb / 4).W), 0.U((p.rvvVlenb / 2).W)),  // 3 of 4
      ...
      val vectorIndices = Option.when(p.enableRvv) {
        ...
        val selected = MuxCase(allIndices, Seq(
            (selectFrom2 && vector.get.dataEmul.curr(0)) -> Cat(0.U((p.rvvVlen / 2).W), allIndices(p.rvvVlen - 1, p.rvvVlen / 2)),
            (selectFrom4 && vector.get.dataEmul.curr(1, 0) === "b01".U) -> Cat(0.U((p.rvvVlen * 3 / 4).W), allIndices(p.rvvVlen / 2 - 1, p.rvvVlen / 4)),
            (selectFrom4 && vector.get.dataEmul.curr(1, 0) === "b10".U) -> Cat(0.U((p.rvvVlen * 3 / 4).W), allIndices(p.rvvVlen * 3 / 4 - 1, p.rvvVlen / 2)),
            (selectFrom4 && vector.get.dataEmul.curr(1, 0) === "b11".U) -> Cat(0.U((p.rvvVlen * 3 / 4).W), allIndices(p.rvvVlen - 1, p.rvvVlen * 3 / 4)),
        ))
        val concatenated = VecInit.tabulate(p.rvvVlenb) { i =>
          ...
          MuxLookup(vector.get.indexEew, WireInit(UInt(32.W), DontCare))(Seq(
        ...
        VecInit.tabulate(p.rvvVlenb) {i =>
          MuxLookup(vector.get.dataEew, WireInit(UInt(32.W), DontCare))(Seq(
      ...
      val ret = MakeWireBundle[State](
          ...
          _.cells -> VecInit.tabulate(nCells) { i =>
            ...
            val cellRespData = MuxUpTo1H(WireInit(UInt(8.W), DontCare), (0 until p.lsuDataBytes).map{ j =>
            ...
            val cellVectorIndex = Option.when(p.enableRvv) {
              MuxUpTo1H(MakeInvalid(UInt(32.W)), (0 until p.rvvVlenb).map{ j =>
                (
                    vectorData.get.valid &&
            ...
            val cellVectorData = Option.when(p.enableRvv) {
              MuxUpTo1H(MakeInvalid(UInt(8.W)), (0 until p.rvvVlenb).map{ j =>
                (
                    vectorData.get.valid &&
                    vectorData.get.bits.vregfile.valid &&
                    vectorDataActive.get(j)(i) &&
            ...
            val cellVectorMask = Option.when(p.enableRvv) {
              MuxUpTo1H(MakeInvalid(Bool()), (0 until p.rvvVlenb).map{ j =>
                (
                    vectorData.get.valid &&
                    vectorData.get.bits.mask.valid &&
                    vectorDataActive.get(j)(i) &&
    ...
    def initVectorUS(
      ...
      ret.vector.foreach { x =>
        // TODO: assert
        x.segmentStep := MuxLookup(elemWidth, WireInit(UInt(3.W), DontCare))(Seq(
          "b000".U -> 1.U,
          "b101".U -> 2.U,
          "b110".U -> 4.U,
        ))
        x.emulStep := MuxLookup(elemWidth, WireInit(UInt(3.W), DontCare))(Seq(
        ...
        x.dataActiveCells := State.makeVectorStartingActiveCells(nfields=nfields, elemWidth=elemWidth, isIndexed=false)
        x.writebackSegment := LoopingCounter(nfields)
        x.writebackEmul := LoopingCounter(maxVectorPerSegment)
        x.writebackActiveCells := MakeValid(State.makeVectorStartingActiveCells(nfields=nfields, elemWidth=elemWidth, isIndexed=false))
      }
    ...
    def initVectorLoadUS(
      ...
      MakeWireBundle[State](
          new State(),
          _ -> initVectorUS(pc, addr, vd, nfields, maxVectorPerSegment, maxVectorPerSegmentOrig, elemWidth, write=false),
          _.cells -> VecInit.tabulate(nCells) { i =>
            Mux(
    ...
    def initVectorStoreUS(
      ...
      MakeWireBundle[State](
          new State(),
          _ -> initVectorUS(pc, addr, vd, nfields, maxVectorPerSegment, maxVectorPerSegmentOrig, elemWidth, write=true),
          _.cells -> VecInit.tabulate(nCells) { i =>
            Mux(
    ...
    def initVectorLoadCS(
      ...
      // TODO: assert
      val offsets = MuxLookup(Cat(nfields, elemWidth), VecInit.fill(nCells)(WireInit(UInt(32.W), DontCare)))(Seq(
          // e8
      ...
      MakeWireBundle[State](
          new State(),
          _ -> initVectorCS(pc, addr, vd, nfields, maxVectorPerSegment, maxVectorPerSegmentOrig, elemWidth, stride, strict, write=false),
          _.cells -> VecInit.tabulate(nCells) { i =>
            Mux(
                // TODO: consider vl/vstart
                (i / p.rvvVlenb).U <= maxVectorPerSegment * nfields + nfields + maxVectorPerSegment,
    ...
    def initVectorStoreCS(
      ...
      MakeWireBundle[State](
          new State(),
          _ -> initVectorCS(pc, addr, vd, nfields, maxVectorPerSegment, maxVectorPerSegmentOrig, elemWidth, stride, strict, write=true),
          _.cells -> VecInit.tabulate(nCells) { i =>
            Mux(
                // TODO: consider vl/vstart
                (i / p.rvvVlenb).U <= maxVectorPerSegment * nfields + nfields + maxVectorPerSegment,
    ...
    def initVectorIndexed(
      ...
      // TODO: assert
      val elemWidthEnum = MuxLookup(elemWidth, WireInit(LsuVectorElementWidth(), DontCare))(Seq(
      ...
      ret.vector.foreach { x =>
        ...
        // TODO: assert
        x.indexEew := MuxLookup(indexWidth, WireInit(LsuVectorElementWidth(), DontCare))(Seq(
            "b000".U -> LsuVectorElementWidth.E8,
        ...
        // This is the number, not lmul/emul encoding
        x.dataEmul := LoopingCounter(maxVectorPerSegment)
        x.dataActiveCells := State.makeVectorStartingActiveCells(nfields=nfields, elemWidth=elemWidth, isIndexed=true)
        x.writebackSegment := LoopingCounter(nfields)
        x.writebackEmul := LoopingCounter(maxVectorPerSegment)
        x.writebackActiveCells := MakeValid(State.makeVectorStartingActiveCells(nfields=nfields, elemWidth=elemWidth, isIndexed=true))
      }
    ...
    def initVectorLoadIndexed(
      ...
      // TODO: assert
      val initMask = State.makeCellInitializationMaskForVector(nfields=nfields, elemWidth=elemWidthEnum, emul=dataEmul)
    ...
    def fromUop(uop: LsuUOp) = {
      ...
      val maxVectorPerSegment = uop.emul_data.map { x =>
        MuxLookup(x, 0.U(3.W))(Seq(
          "b001".U -> 1.U(3.W),
          "b010".U -> 3.U(3.W),
          "b011".U -> 7.U(3.W),
        ))
      }.getOrElse(0.U)
      val maxVectorPerSegmentOrig = uop.emul_data_orig.map { x =>
        MuxLookup(x, 0.U(3.W))(Seq(
          "b001".U -> 1.U(3.W),
          "b010".U -> 3.U(3.W),
          "b011".U -> 7.U(3.W),
      ...
      val subvectors = uop.emul_data.map { x =>
        MuxLookup(Cat(uop.sew.getOrElse(0.U), elemWidth, x), 0.U(3.W))(Seq(
          // e8ei16
      ...
      val subvectorsTheoretical = uop.emul_data.map { x =>
        MuxLookup(Cat(uop.sew.getOrElse(0.U), elemWidth), 0.U(3.W))(Seq(
          // e8ei16
          "b000_101".U -> 1.U(2.W),
          // e16ei32
          "b001_110".U -> 1.U(2.W),
  ...
  object State {
    ...
    def makeCellInitializationMaskForVector(
      ...
      val nStructs = MuxUpTo1H(0.U, Seq(
          // mf4
      ...
      val structBytes = MuxLookup(elemWidth, 0.U)(Seq(
    ...
    def makeVectorStartingActiveCells(
      ...
      if (isIndexed) {
        // This is SEW.
        MuxLookup(elemWidth, VecInit.fill(p.rvvVlenb)(WireInit(UInt(indexWidth.W), DontCare)))(Seq(
          "b000".U -> retE8,
          "b001".U -> retE16,
          "b010".U -> retE32,
        ))
  ...
  // TODO: hold the tx as needed
  io.busReq <> IrrevocableChecker(MakeIrrevocable(Mux(
    // TODO: If we allow an instruction to generate multiple faults (we should,
    // if we pipeline the bus), we only need to block based on state.faulted
    // here, which can be easier on timing.
...
class LsuV3(p: Parameters) extends Lsu(p) {
  ...
  val rs = Module(new CircularBufferMulti(new LsuUOp(p), p.instructionLanes, math.max(4, p.instructionLanes)))
  ...
  val itcm = p.m.filter(_.memType == MemoryRegionType.IMEM)
                .map(_.contains(addr)).reduceOption(_ || _).getOrElse(false.B)
  val dtcm = p.m.filter(_.memType == MemoryRegionType.DMEM)
                .map(_.contains(addr)).reduceOption(_ || _).getOrElse(true.B)
  val peri = p.m.filter(_.memType == MemoryRegionType.Peripheral)
                .map(_.contains(addr)).reduceOption(_ || _).getOrElse(false.B)
  ...
  val faultReg = RegNext(MuxCase(MakeInvalid(new FaultInfo(p)), Seq(
      io.ebus.fault.valid -> io.ebus.fault,
      ibusFault.valid -> ibusFault,
  ...
  io.ebus.dbus.size := 16.U  // TODO: limit this
  ...
  slot.io.busResp.bits.rdata := MuxUpTo1H(WireInit(UInt(p.lsuDataBits.W), DontCare), Seq(
  ...
  slot.io.busReq.ready := MuxUpTo1H(false.B, Seq(
